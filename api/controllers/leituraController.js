...
class leiturasController {
    ...
    static async listar(req, res) {
        try {
            ...
            // !!!! obs2: MUDAR O NOME DA TABELA PARA ***leituras_sensor*** senao nao vai funcionar !!!!!
            const leituras = await BD.query(`
                SELECT leituras_sensor.valor, leituras_sensor.id_sensor, leituras_sensor.data_hora_leitura, s.nome AS sensor, s.unidade, s.tipo, c.valor AS cultivo
                    FROM leituras_sensor
                    INNER JOIN sensores AS s
                    INNER JOIN cultivo AS c 
                    ON leituras_sensor.id_sensor = s.id_sensor
                    WHERE s.ativo = true
    ...
    static async editar(req, res) {
        ...
        try {
            ...
            const query = `UPDATE leituras_sensor
                            SET ${campos.join(", ")}
